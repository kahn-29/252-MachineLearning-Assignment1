...
Update `backbones.py` and `config_utils.py` (`SUPPORTED_BACKBONES`). Support `get_backbone`, `get_feature_dim`, and `build_transfer_model`.

### Add a New Classifier

Update `classical_models.py` and `config_utils.py` (`SUPPORTED_CLASSIFIERS`). Support `get_classifier` and `get_param_grid`.

### Add a New Preprocessing Mode

Update `transforms.py` and `config_utils.py` (`SUPPORTED_PREPROCESSING_MODES`). Support `get_hybrid_transform` and `build_image_transform`.

### Add a New Experiment Type

Place orchestration in `grid_search.py` rather than inside notebooks.
