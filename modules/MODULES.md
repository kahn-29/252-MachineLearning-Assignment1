...
| `copy_dataframe_images(df, output_dir, path_col="path", label_col="label_name", overwrite=False)` | Dataframe and export path | `pd.DataFrame` | Copies indexed images to `output_dir/<label_name>/`; useful for inspection/export. |
...
| `create_run_dir(base_dir, run_name=None)` | Base dir and optional run name | `Path` | Creates and returns run directory. |
| `save_experiment_result(result, output_dir, run_name)` | Structured result dict, output dir, run name | `dict[str, Path]` | Saves metrics/config/dataframes/summary to files. |
...
| Function | Input | Output | Meaning |
|---|---|---|---|
| `savefig(path, dpi=160, bbox_inches="tight")` | Path and save options | `Path` | Saves current matplotlib figure. Compatibility helper. |
...
| `plot_class_distribution_pie(df, label_col="label_name", title="Class Distribution", save_path=None, show=False)` | Dataframe | `tuple[Figure, Axes]` | Pie chart of class distribution. |
| `plot_class_distribution_bar(df, label_col="label_name", title="Class Distribution", save_path=None, show=False)` | Dataframe | `tuple[Figure, Axes]` | Bar chart of class counts and percentages. |
| `plot_image_size_distribution(audit_df, width_col="width", height_col="height", label_col="label_name", title="Raw Image Size Distribution", save_path=None, show=False)` | Audit dataframe | `tuple[Figure, Axes]` | Width-height scatter plot. |
...
| `plot_training_curves(history_df, save_path=None, epoch_col=None, show=False)` | Training history dataframe | `tuple[Figure, np.ndarray]` | Plots train/val accuracy/loss/F1 curves. |
| `plot_model_comparison(comparison_df, metric="f1_macro", model_col="model", title=None, save_path=None, show=False)` | Model comparison dataframe | `tuple[Figure, Axes]` | Bar chart comparing models by metric. |
