...
| `tune_classifier_grid(X_train, y_train, classifier_name, param_grid, cv=3, seed=42, scoring="f1_macro", n_jobs=-1)` | Training data and hyperparameter grid     | `GridSearchCV`    | Runs sklearn grid search for one classifier. Automatically fetches default grid based on `grid_size` if `param_grid` is omitted. |
